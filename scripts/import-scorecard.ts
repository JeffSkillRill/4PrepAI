...
function sqlMigration(ranked: School[], eligible: School[], config: Config): { sql: string; known: number; unknown: number } {
  const ranking = `us-4prep-ranking-scorecard-${config.dataYear}`, lines = ["begin;", "", "-- Generated by scripts/import-scorecard.ts. Additive only; no deletes.", "alter table public.universities add column if not exists rank int;", "alter table public.universities add column if not exists rank_source_id text references public.sources(id);", "alter table public.universities add column if not exists unitid int;", "alter table public.universities add column if not exists state text;", "", "insert into public.sources (id, name, url, retrieved_at, verification) values", `  (${q(ranking)}, ${q(`4Prep ranking — computed from College Scorecard (${config.dataYear})`)}, 'https://collegescorecard.ed.gov/data/', ${q(nowDate())}, 'verified')`, "on conflict (id) do nothing;", ""];
  let known = 0, unknown = 0; ranked.forEach((s) => { const x = writeSchool(lines, s, config); known += x.known; unknown += x.unknown; });
  const rows = Object.entries(config.existingSchoolUnitids).sort(([a], [b]) => nameCompare(a, b)).map(([id, unitid]) => {
    const school = eligible.find((item) => item.unitid === unitid);
    return `  (${q(id)}, ${unitid}, ${school?.rank ?? null}, ${q(school?.state ?? null)})`;
  });
  lines.push("-- The only write to the 18 existing four-year catalogue rows: rank provenance, UNITID, and source-record state.", "update public.universities as university", "set rank = ranked.rank,", `    rank_source_id = case when ranked.rank is null then null else ${q(ranking)} end,`, "    unitid = ranked.unitid,", "    state = ranked.state", "from (values", rows.join(",\n"), ") as ranked(id, unitid, rank, state)", "where university.id = ranked.id", `  and university.id in (${[...EXISTING_SLUGS].sort(nameCompare).map(q).join(", ")});`, "", "-- Required because this materialized view has no insert trigger.", "refresh materialized view public.university_search_index;", "commit;", "");
