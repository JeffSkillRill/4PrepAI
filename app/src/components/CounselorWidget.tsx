      ...
      {open ? (
        <section className="counselor-widget-panel w-[calc(100vw-2rem)] max-w-sm overflow-hidden rounded-xl border border-forest-200 bg-white shadow-raised" aria-live="polite">
          ...
          <div className="max-h-72 overflow-y-auto px-4 py-3 text-sm leading-6 text-ink" aria-live="polite">
            {turns.length > 0 ? <div className="space-y-3">{turns.map((turn) => turn.type === 'comparison' ? <div key={turn.id} className="rounded-xl bg-forest-50 px-3 py-2 text-forest-900"><p className="text-[10px] font-extrabold uppercase tracking-[.12em] text-forest-700">4Prep counselor</p><p className="mt-1 font-bold">University comparison added</p><p className="mt-1 text-xs text-muted">Open the full chat to review the current sourced comparison.</p></div> : <div key={turn.id} className="space-y-2"><div className="ml-5 rounded-xl rounded-br-sm bg-forest-800 px-3 py-2 text-white"><p className="text-[10px] font-extrabold uppercase tracking-[.12em] text-white/65">You</p><p>{turn.question}</p></div><div className="mr-5 rounded-xl rounded-bl-sm bg-canvas px-3 py-2"><p className="text-[10px] font-extrabold uppercase tracking-[.12em] text-muted">4Prep counselor</p>{turn.answer ? <SafeMarkdown text={turn.answer.answer} className="mt-1" /> : <p className="mt-1 text-rose-900">{turn.error ?? 'No verified answer is available right now.'}</p>}</div></div>)}</div> : <p>Hi, I can help you understand verified university information. What would you like to know?</p>}
