...
export function ToolsScreen({ onNavigate }: { onNavigate: (view: View) => void }) {
  ...
  return (
    <div>
      ...
      <section className="page-container py-10 lg:py-14"><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{tools.map((tool, index) => { const Icon = toolIcons[index]; return <ToolCard key={tool.id} tool={tool} icon={<Icon size={25} />} href={tool.view ? viewPaths[tool.view] : undefined} onOpen={() => tool.view ? onNavigate(tool.view) : undefined} /> })}</div>
        <div className="mt-10 grid gap-6 rounded-2xl border border-line bg-white p-6 shadow-soft sm:grid-cols-[1fr_auto] sm:items-center"><div className="flex items-start gap-4"><div className="grid size-12 shrink-0 place-items-center rounded-xl bg-amber-100 text-amber-800"><LockKeyhole size={22} /></div><div><h2 className="display text-xl font-extrabold">Your private planning data stays yours</h2><p className="mt-1 text-sm leading-6 text-muted">Sign in to restore your intake profile and saved plans. Database row-level security limits both to your account.</p></div></div><AppLink href={viewPaths.auth as string} onNavigate={() => onNavigate('auth')} className="rounded-xl border border-line px-4 py-3 text-sm font-bold text-forest-800">Open account</AppLink></div>
      </section>
    </div>
  )
}

function ToolCard({ tool, icon, href, onOpen }: { tool: ToolItem; icon: React.ReactNode; href?: string; onOpen: () => void | undefined }) {
  return <article className="card interactive-card group p-6"><div className="flex items-start justify-between"><div className="grid size-12 place-items-center rounded-xl bg-forest-50 text-forest-700">{icon}</div><span className="rounded-full bg-canvas px-3 py-1.5 text-xs font-bold text-muted">{tool.tag}</span></div><h2 className="display mt-6 text-2xl font-extrabold">{tool.name}</h2><p className="mt-3 min-h-12 text-sm leading-6 text-muted">{tool.description}</p>{href ? <AppLink href={href} onNavigate={onOpen} className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-forest-700 transition group-hover:gap-3">Open tool <ArrowRight size={16} /></AppLink> : null}</article>
}

export function SavedScreen({ saved, onToggleSave, onOpen, onExplore, onOpenCounselor }: { saved: Set<string>; onToggleSave: (id: string) => void; onOpen: (university: University) => void; onExplore: () => void; onOpenCounselor: () => void }) {
  ...
  return (
    <div className="page-container py-10 lg:py-14">
      <div className="flex flex-wrap items-end justify-between gap-5"><div><p className="text-sm font-bold uppercase tracking-[.14em] text-forest-700">Saved</p><h1 className="display mt-2 text-4xl font-extrabold sm:text-5xl">Your considered shortlist</h1><p className="mt-4 max-w-2xl leading-7 text-muted">Keep promising routes together, then compare their evidence when you are ready.</p></div>{items.length > 1 && <button type="button" onClick={onOpenCounselor} className="inline-flex items-center gap-2 rounded-xl bg-forest-800 px-5 py-3 font-bold text-white"><GitCompareArrows size={18} /> Compare in counselor</button>}</div>
      {items.length > 0 ? <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{items.map((university, index) => <UniversityCard key={university.id} university={university} revealIndex={index} saved onSave={() => onToggleSave(university.id)} onOpen={() => onOpen(university)} />)}</div> : <section className="soft-grid mt-9 overflow-hidden rounded-[28px] border border-line bg-white px-6 py-14 text-center shadow-soft sm:px-12"><div className="mx-auto grid size-24 place-items-center rounded-full bg-forest-50 text-forest-700"><BookMarked size={52} strokeWidth={1.4} /></div><h2 className="display mt-7 text-3xl font-extrabold">Save routes worth a second look</h2><p className="mx-auto mt-3 max-w-lg leading-7 text-muted">Use the bookmark on any university card. Your account’s private shortlist will appear here.</p><AppLink href={viewPaths.search as string} onNavigate={onExplore} className="mt-7 inline-flex items-center gap-2 rounded-xl bg-forest-800 px-5 py-3 font-bold text-white">Explore universities <ArrowRight size={18} /></AppLink></section>}
