...
import type { DevState, University, View } from './types'
  ...
  return (
    <div className="border-b border-amber-200 bg-amber-soft text-amber-950">
      <div className="page-container flex min-h-9 items-center justify-between gap-3 py-1.5 text-xs">
        <p className="flex items-center gap-2"><Sparkles size={14} className="shrink-0" /><span><strong>SAMPLE DATA</strong><span className="hidden sm:inline"> · Fictional universities for product demonstration only</span></span></p>
...
export default function App() {
  ...
  const [saved, setSaved] = useState<Set<string>>(new Set(['northbridge', 'tulip']))

  useEffect(() => { window.scrollTo({ top: 0 }) }, [view])

  const navigate = (next: View) => setView(next)
  const openUniversity = (university: University) => { setSelectedUniversity(university); setView('profile') }
  ...
  if (devState === 'loading') screen = <LoadingState />
    ...
    screen = <>
      ...
      {view === 'profile' && <ProfileScreen university={selectedUniversity} saved={saved.has(selectedUniversity.id)} onToggleSave={() => toggleSave(selectedUniversity.id)} />}
      {view === 'compare' && <CompareScreen />}
      {view === 'intake' && <IntakeScreen onComplete={() => setView('results')} />}
      {view === 'results' && <ResultsScreen onOpen={openUniversity} />}
