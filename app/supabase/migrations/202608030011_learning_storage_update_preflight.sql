...
-- Supabase Storage uses the same `contentLength` metadata shape when it checks
-- an object update/upsert before streaming bytes. Keep the existing UPDATE
-- owner, path, MIME, extension, and bucket checks while accepting both the
-- preflight and completed-object size keys.
alter policy "learning submission object owner can update allowed file"
on storage.objects
with check (
